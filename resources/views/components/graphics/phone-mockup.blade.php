...
- device: 'iphone-14-pro' | 'iphone-14' | 'iphone-x' | 'galaxy-s8'
- color: devices.css color class (e.g. 'device-silver', 'device-gold')
...
@php
    ...
    // Device native dimensions for container sizing
    $deviceDims = [
        'iphone-14-pro' => ['w' => 428, 'h' => 868],
        'iphone-14'     => ['w' => 416, 'h' => 848],
        'iphone-x'      => ['w' => 375, 'h' => 812],
        'galaxy-s8'     => ['w' => 395, 'h' => 829],
    ];
    $dims = $deviceDims[$device] ?? $deviceDims['iphone-14-pro'];
    $containerW = round($dims['w'] * $scale);
    $containerH = round($dims['h'] * $scale);
@endphp

<div {{ $attributes->merge(['class' => 'relative']) }}
     style="width: {{ $containerW }}px; height: {{ $containerH }}px;">
    <div class="{{ $color ? "device device-{$device} {$color}" : "device device-{$device}" }}"
                ...
                @if($slot->isEmpty())
                    <div class="relative h-full w-full">
                        ...
                        @elseif($variant === 'error')
                            <div class="absolute inset-0 flex flex-col items-center justify-center {{ $image ? 'bg-neutral-900/70 backdrop-blur-sm' : 'bg-neutral-900' }} p-12">
                                <div class="mb-8 flex h-14 w-14 items-center justify-center rounded-full bg-red-500/20">
